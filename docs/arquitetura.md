...
O **Monitora Imóveis** separa **frontend** (Next.js), **backend** (FastAPI) e **persistência** (SQLite). A autenticação é feita pelo **Clerk** no browser; o **FastAPI** valida o JWT e aplica **multi-tenant** por `user_id` em `Property`. A atualização periódica em massa (**cron** interno) ainda não está implementada; o fluxo principal hoje é **cadastro sob demanda** + **leitura** da lista.
...
- **`Property`:** `user_id` (string Clerk), URL **única por usuário** (constraint composta `user_id` + `url`), demais dados do anúncio, `status` de saúde do scrape (`active` / `inactive` / `error`, conforme modelo).
...
| `frontend/src/lib/api.ts` | Chamadas HTTP com Bearer |

...
import { CalendarCheck, BarChart2, TrendingUp, Sigma, AlertTriangle, Info } from "lucide-react";
...
const LocalEtlSection = () => {
  ...
  const dadosDesatualizados = verificarDadosDesatualizados(kpis.ultimaAtualizacao);
  ...
  if (period === "7d") {
    ...
    const dadosReais = etlData.sort((a, b) => a.data_etl.localeCompare(b.data_etl));
    ...
    <div className="w-full mt-6 flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <Badge variant="outline" className="flex items-center gap-1">
            <Info size={14} />
            Última atualização: {kpis.ultimaAtualizacao ? formatarDataEtl(kpis.ultimaAtualizacao) : 'N/A'}
          </Badge>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <EtlKpiCard
          ...
          description={kpis.ultimoDia?.data_etl ? formatarDataEtl(kpis.ultimoDia.data_etl) : 'N/A'}
          icon={<CalendarCheck size={18} />}
        />
        <EtlKpiCard
          title={`Média Diária (${period === "all" ? kpis.dadosCompletos.length : period.replace("d", "")
            }d)`}
          value={kpis.media}
        ...
        />
        <EtlKpiCard
          title={`Pico de Processamento (${period === "all" ? kpis.dadosCompletos.length : period.replace("d", "")
            }d)`}
          value={kpis.pico}
          sparkData={filteredData}
      ...
      <div className="bg-gray-50 p-4 rounded-lg">
        <h3 className="font-semibold text-gray-700 mb-2">Informações do Período</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <span className="text-gray-600">Período solicitado:</span>
            <p className="font-medium">{period === "all" ? "Todos os dados" : `${period.replace("d", "")} dias`}</p>
          </div>
          <div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-200">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              ...
              <p className="font-medium">{kpis.diasComDados + kpis.diasSemDados} dia(s)</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-medium">
                {kpis.ultimaAtualizacao ? formatarDataEtl(kpis.ultimaAtualizacao) : 'N/A'}
              </p>
            </div>
          </div>
        </div>
